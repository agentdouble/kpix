...
- `src/pages` : écrans du parcours manager (dont la page détail KPI avec un graphique d'évolution Chart.js interactif qui met en évidence les périodes avec actions/commentaires : les échéances d’actions ouvertes ou en cours sont projetées sur les périodes du KPI en fonction de sa fréquence, et une gestion des plans d’action assignés à des membres avec avancement et statut).
